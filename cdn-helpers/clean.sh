...
print_help () {
    ...
    echo ""
    echo "Examples:"
    echo "  sh clean.sh --port=22 --destination=cdndoppler@reporting.fromdoppler.com:/cdndoppler/doppler-style-guide/ --environment=main"
    echo "  sh clean.sh --port=22 --destination=cdndoppler@reporting.fromdoppler.com:/cdndoppler/doppler-style-guide/ --environment=pr --keep-days=7"
}
...
deleteDocumentationDirs="${tmpDir}/delete-documentation-dirs"
...
extract_manifest_assets () {
  ...
  awk '
    /"entrypoints"[[:space:]]*:/ { in_entrypoints=1 }
    in_entrypoints {
      while (match($0, /static\/[^"]+\.(css|js)/)) {
  ...
  ' "${manifest}" | while IFS= read -r asset
  do
    [ -n "${asset}" ] || continue
...
if [ "${environment}" = "pr" ]
then
  sed -n 's/^asset-manifest-\(pr-[0-9][0-9]*\)\(-.*\)\{0,1\}\.json$/documentation\/\1/p' "${deleteManifests}" \
    | sort -u > "${deleteDocumentationDirs}"
fi
...
deleteDocumentationDirsCount="$(wc -l < "${deleteDocumentationDirs}" | tr -d ' ')"
...
if [ "${environment}" = "pr" ] && [ "${deleteDocumentationDirsCount}" -gt 0 ]
then
  echo "Deleting ${deleteDocumentationDirsCount} old PR documentation directories..."
fi
...
if [ "${environment}" = "pr" ] && [ "${deleteDocumentationDirsCount}" -gt 0 ]
then
  echo "Deleted ${deleteDocumentationDirsCount} old PR documentation directories."
fi
